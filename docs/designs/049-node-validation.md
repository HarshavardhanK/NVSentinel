...
* **schedulingGate:** groups all node scheduling controls applied by the validation-controller during the lifecycle of a validation request.
    * **cordon.add:** indicates whether nodes needing validation should be cordoned by the validation-controller if not already cordoned. The cordon will only be applied after the readinessCriteria is satisfied.
    ...
    * **taints:** the list of taints the controller manages on nodes under validation. Each taint specifies a key, an optional value, an effect, and add/remove booleans controlling whether the controller applies and removes the taint during the validation lifecycle. Taints with add set to true are applied after the readinessCriteria is satisfied, in the same step as cordon.add.
    * **tolerations:** all test pods created by the validation-controller will automatically tolerate all taints (rather than need an explicit toleration for the unschedulable taint and for every taint listed in taints). Note that these pod tolerations will be applied regardless of whether those taints are present on the node. This covers the case where a node was cordoned or tainted by an operator manually, by the validation-controller, or not cordoned or tainted at all. This ensures test pods can always be scheduled on nodes that are gated from regular workloads.
* **readinessCriteria:** a set of CEL expressions which must all evaluate to true before a validation test can be started on a given node. This is primarily needed for new nodes undergoing validation to ensure that all GPU Operator and NVSentinel operands are ready on the given node.
    ...
    * If an operator is externally applying a node cordon or taint without cordon.add or taints[*].add and would like to block validation on these being applied, they could add these properties to the readinessCriteria.
